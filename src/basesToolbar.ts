...
async function getHiddenTargetsForContext(plugin: Plugin, ctx: MarkdownPostProcessorContext): Promise<Set<string>> {
	...
	if (!(file instanceof TFile) || file.extension !== 'md') return new Set<string>();
...
function computeBaseNameAndNextFlag(rawLabel: string, normalizedPath: string): { baseName: string; nextFlag: 'x' | 'o' } {
	...
	// 切换状态：x -> o（解锁），其他 -> x（上锁）
	const nextFlag: 'x' | 'o' = isLocked ? 'o' : 'x';

	const baseName =
		parts.length > 0 ? parts.join('|') : deriveNameFromPath(normalizedPath);

	return { baseName, nextFlag };
}

function updateEmbedDomAfterToggle(
